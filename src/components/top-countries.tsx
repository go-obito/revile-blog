...
export function TopCountries({ rows: dataRows }: { rows?: Array<{ code: string; visits: number; delta: number }> }) {
	...
	return (
		<Card className="relative md:col-span-2 dark:bg-transparent">
			<CardHeader>
				...
				<CardDescription className="text-pretty">Top countries in the last 12 months.</CardDescription>
			</CardHeader>
			<CardContent className="mask-b-from-50% mask-b-to-100% p-0 pb-2">
				{rowsToRender.length ? (
					<Table className="border-t">
						<TableCaption className="sr-only">Top countries by visits with year-over-year change.</TableCaption>
						<TableHeader>
							<TableRow>
								...
								<TableRow className="hover:bg-transparent" key={row.code}>
									<TableCell className="max-w-[220px] truncate pl-6 font-medium">
										<span className="inline-flex max-w-full items-center gap-2">
											<img alt={`Flag of ${row.code}`} className="h-3.5 w-5 shrink-0 rounded object-cover" height={14} src={flagUrl(row.code)} width={20} />
											<span className="min-w-0 truncate text-xs">{regionNames.of(row.code) ?? row.code}</span>
										</span>
									</TableCell>
					...
					<div className="flex min-h-[180px] items-center justify-center px-6 text-sm text-muted-foreground">No country traffic data yet.</div>
				)}
			</CardContent>
